...
### Spawn-region randomization (libero_object)

To stress-test a policy's spatial generalization, the target object's spawn region can be resampled
every episode instead of using the suite's default placement. This only works for suites that ship a
`spawn_region.json` next to their bddl files — today that's `libero_object` only:
...
lerobot-eval \
  ...
  --env.spawn_train_distribution=false \
  --eval.batch_size=2 \
  --eval.n_episodes=10
```

- `--env.change_spawn` (default `false`) — when `true`, every episode rebuilds the sim from a bddl
  file with a freshly-sampled `floor_target_object_region`.
- `--env.spawn_train_distribution` (default `true`) — `true` samples an alternate in-distribution
  region (the `"target"` pool in `spawn_region.json`); `false` samples an out-of-distribution region
  taken from other objects' regions (the `"other_objects"` pool), swapping that object's region with
  the target's vacated one so they don't end up overlapping.

This rebuilds the MuJoCo scene on every reset, so expect eval to run noticeably slower than with
`--env.change_spawn=false`.

<Tip warning={true}>

`init_states` defaults to `true`, and takes priority over spawn randomization: after the sim is
rebuilt from the resampled bddl file, `reset()` would otherwise immediately call
`set_init_state()` with a fixed, recorded demo state, overwriting object positions (among the rest
of the sim state) and silently undoing the resampled region. `LiberoEnv.reset()` now skips
`set_init_state()` whenever `change_spawn=True`, so no extra flag is needed — but if you're
running against an older checkout, add `--env.init_states=false` explicitly.
